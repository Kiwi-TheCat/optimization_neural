...
original_loss = evaluate_final_loss(original_params, X_train, relu);
...
for i = 1:length(Layer_Names)
    for j = 1:length(Noise_Levels)
        ...
        initial_loss = evaluate_final_loss(noisy_params, X_train, relu);
        fprintf('Initial loss with noisy %s: %.6f\n', current_layer, initial_loss);
        
        % Train the noisy weights using Adam optimizer
        [final_params, loss_history] = train_with_adam(noisy_params, X_train, learning_rate, num_epochs, adam_beta1, adam_beta2, adam_epsilon, relu, relu_derivative);
        ...
        final_loss = evaluate_final_loss(final_params, X_train, relu);
...
%% Save training results for convergence analysis
save('trained_noisy_weights.mat', 'results', 'loss_histories', 'original_params', 'final_params_all');
fprintf('\nTraining results saved to trained_noisy_weights.mat\n');

%% Supporting Functions

function [final_params, loss_history] = train_with_adam(initial_params, X_train, lr, epochs, beta1, beta2, epsilon, relu, relu_deriv)
    % Initialize weights
    We1 = initial_params.We1;
    be1 = initial_params.be1;
    We_latent = initial_params.We_latent;
    be_latent = initial_params.be_latent;
    Wd1 = initial_params.Wd1;
    bd1 = initial_params.bd1;
    Wd_output = initial_params.Wd_output;
    ...
    final_params = struct('We1', We1, 'be1', be1, 'We_latent', We_latent, 'be_latent', be_latent, 'Wd1', Wd1, 'bd1', bd1, 'Wd_output', Wd_output, 'bd_output', bd_output);
end

function loss = evaluate_final_loss(params, X_train, relu_func)
    total_loss = 0;
    num_samples = size(X_train, 1);
    
    for i = 1:num_samples
        X_sample = X_train(i, :);
        [~, ~, sample_loss] = forward_pass(X_sample, params.We1, params.be1, params.We_latent, params.be_latent, params.Wd1, params.bd1, params.Wd_output, params.bd_output, relu_func);
